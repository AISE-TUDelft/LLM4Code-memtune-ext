"""
This script is used to fine-tune StarCoder2 family models on a java dataset, for code completion task using the LoRA method.
"""
import torch
...
from peft import LoraModel, LoraConfig, get_peft_model, AutoPeftModelForCausalLM
import deepspeed
...
wproject = "memtune_peft" # wb project name
run_name = "LoRA_7B" # name of the W&B run (optional)
# training batches
batch = 1
...
train_20 = dataset_train_20.select_columns(text_column)
validation_20 = dataset_valid_20.select_columns(text_column)
...
num_train_epochs = 3
eval_steps = 0.25 #200 # each epoch two evaluations
...
training_args = TrainingArguments(
    ...
    #ddp_find_unused_parameters=False,  # Faster with LoRA (ignores unused layers)
